...
class Media {
  extra = 0
  geometry: any; gl: GL; image: string; index: number; length: number
...
class App {
  ...
  medias: Media[] = []; mediasImages: any[] = []; screen!: any; viewport!: any; raf = 0
  boundOnResize!: () => void; boundOnWheel!: (e: Event) => void
  boundOnTouchDown!: (e: any) => void; boundOnTouchMove!: (e: any) => void; boundOnTouchUp!: () => void
  isDown = false; start = 0

  constructor(container: HTMLElement, { items, bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px sans-serif', scrollSpeed = 2, scrollEase = 0.05 }: any) {
...
function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px sans-serif', scrollSpeed = 2, scrollEase = 0.05 }: any) {
...
interface CurvedLoopProps {
  marqueeText?: string; speed?: number; curveAmount?: number
  direction?: 'left' | 'right'; interactive?: boolean
}
const CurvedLoop: FC<CurvedLoopProps> = ({ marqueeText = '', speed = 2, curveAmount = 0, direction = 'left', interactive = true }) => {
  const text = useMemo(() => {
  ...
  const dragRef = useRef(false); const lastXRef = useRef(0)
  const dirRef = useRef<'left' | 'right'>(direction); const velRef = useRef(0)
  ...
  useEffect(() => {
    if (!spacing) return
    if (textPathRef.current) { textPathRef.current.setAttribute('startOffset', -spacing + 'px'); setOffset(-spacing) }
  }, [spacing])
  useEffect(() => {
    ...
    const step = () => {
      if (!dragRef.current && textPathRef.current) {
        ...
        textPathRef.current.setAttribute('startOffset', next + 'px'); setOffset(next)
      }
  ...
  const endDrag = () => {
    if (!interactive) return
    dragRef.current = false; dirRef.current = velRef.current > 0 ? 'right' : 'left'
  }
  return (
    <div style={{ visibility: ready ? 'visible' : 'hidden', cursor: interactive ? 'grab' : 'auto', width: '100%', display: 'flex', alignItems: 'center', height: '100%' }}
      onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endDrag} onPointerLeave={endDrag}>
      <svg style={{ userSelect: 'none', width: '100%', overflow: 'visible', display: 'block' }} viewBox="0 0 1440 80">
        <text ref={measureRef} xmlSpace="preserve" style={{ visibility: 'hidden', opacity: 0, pointerEvents: 'none', fontFamily: "'Bebas Neue',sans-serif", fontSize: '2.2rem', fontWeight: 700 }}>{text}</text>
        <defs><path ref={pathRef} id={pathId} d={pathD} fill="none" stroke="transparent" /></defs>
        {ready && (
          <text xmlSpace="preserve" style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '2.2rem', fontWeight: 700, letterSpacing: '0.07em', fill: '#b5f02a' }}>
...
// ══════════════════════════════════════════════════════
const WORDS = [
  { word: 'ସାଥୀ', lang: 'Odia' }, { word: 'FRIEND', lang: 'English' },
  { word: 'साथी', lang: 'Hindi' }, { word: 'दोस्त', lang: 'Hindi' },
  { word: 'ALLY', lang: 'English' }, { word: 'ସଖା', lang: 'Odia' },
  { word: 'COMPANION', lang: 'English' }, { word: 'यार', lang: 'Hindi' },
  { word: 'ବନ୍ଧୁ', lang: 'Odia' },
]

const GALLERY_ITEMS = [
  { image: 'https://images.unsplash.com/photo-1500937386664-56d1dfef3854?w=800&q=80', text: 'Wheat Fields' },
  { image: 'https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?w=800&q=80', text: 'Rice Harvest' },
  { image: 'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=800&q=80', text: 'Fresh Produce' },
  { image: 'https://images.unsplash.com/photo-1560493676-04071c5f467b?w=800&q=80', text: 'Livestock Care' },
  { image: 'https://images.unsplash.com/photo-1464226184884-fa280b87c399?w=800&q=80', text: 'Farm Life' },
  { image: 'https://images.unsplash.com/photo-1523741543316-beb7fc7023d8?w=800&q=80', text: 'Crop Fields' },
  { image: 'https://images.unsplash.com/photo-1625246333195-78d9c38ad449?w=800&q=80', text: 'Paddy Season' },
  { image: 'https://images.unsplash.com/photo-1584677626646-7c8f83690304?w=800&q=80', text: 'Soil & Seeds' },
]
...
export default function HomePage() {
  ...
  const [videoOpen, setVideoOpen] = useState(false)   // ← video modal state
  ...
  const wordAnim: React.CSSProperties = {
    ...
    transition: leaving ? 'transform .42s cubic-bezier(.55,0,.78,0),opacity .35s ease' : entered ? 'transform .56s cubic-bezier(.22,1,.36,1),opacity .44s ease' : 'none',
  }
  const langAnim: React.CSSProperties = {
    ...wordAnim,
    transition: leaving ? 'transform .42s .04s cubic-bezier(.55,0,.78,0),opacity .35s ease' : entered ? 'transform .56s .06s cubic-bezier(.22,1,.36,1),opacity .44s ease' : 'none',
  }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .marquee-strip { width: 100%; height: 50px; overflow: hidden; background: #2F4F3A; border-bottom: 1px solid rgba(181,240,42,0.2); display: flex; align-items: center; padding-top: 18px; }
        .gallery-section { width: 100%; height: 520px; background: #7F8C63; border-top: 1px solid rgba(181,240,42,0.08); position: relative; overflow: hidden; }
        .gallery-header { position: absolute; top: 32px; left: 48px; right: 48px; display: flex; align-items: baseline; gap: 20px; z-index: 2; pointer-events: none; }
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", background: '#0d1a0d', color: '#f4f4f0', minHeight: '100vh', overflowX: 'hidden' }}>
        ...
        <div className="marquee-strip">
          <CurvedLoop marqueeText="🌾 KRISHI MITRA &nbsp;•&nbsp; YOUR FARMING COMPANION &nbsp;•&nbsp; AI POWERED &nbsp;•&nbsp; ODISHA FARMERS &nbsp;•&nbsp; CROP DOCTOR &nbsp;•&nbsp; LIVESTOCK CARE &nbsp;•&nbsp;" speed={1.4} curveAmount={0} direction="left" interactive={true} />
        </div>

        {/* ══ HERO ══ */}
        <section className="hero-section hero-bg" style={{ position: 'relative', minHeight: 'calc(100vh - 72px)', display: 'grid', gridTemplateColumns: '1fr 1fr', padding: '0 48px', overflow: 'hidden' }}>
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 0 80px', gap: 28 }}>
            <div className="heading-row" style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
              <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(4rem,10vw,9.5rem)', lineHeight: 1, letterSpacing: '0.01em', color: '#0d1a0d', textTransform: 'uppercase', whiteSpace: 'nowrap', textShadow: 'none' }}>KRISHI</h1>
              <div className="vert-line" style={{ width: 3, height: 'clamp(4rem,10vw,9.5rem)', background: 'linear-gradient(180deg,rgba(181,240,42,.12) 0%,#b5f02a 45%,rgba(181,240,42,.12) 100%)', borderRadius: 2, flexShrink: 0 }} />
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
                <div style={{ overflow: 'hidden', height: 'clamp(3.8rem,9.8vw,9rem)' }}>
                  <span key={`w-${index}`} style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(3.8rem,9.8vw,9rem)', lineHeight: 1, color: '#1a5c0a', textTransform: 'uppercase', letterSpacing: '0.01em', textShadow: 'none', ...wordAnim }}>
                    {WORDS[index].word}
                  </span>
                </div>
                <div style={{ overflow: 'hidden', height: '1.1rem' }}>
                  <span key={`l-${index}`} style={{ display: 'block', fontSize: '0.68rem', fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#1a5c0a', ...langAnim }}>
                    {WORDS[index].lang}
                  </span>
                </div>
              </div>
            ...
            <p style={{ fontSize: '0.95rem', color: '#1a2e10', maxWidth: 360, lineHeight: 1.78, fontWeight: 500, textShadow: 'none' }}>
              Modern solutions, AI technology, and expert support to help Odisha farmers grow more.
            </p>

            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              <Link href="/livestock"
                style={{ padding: '13px 28px', background: '#0d1a0d', color: '#f4f4f0', fontSize: '0.85rem', fontWeight: 600, letterSpacing: '0.04em', textDecoration: 'none', transition: 'background .18s,transform .15s', display: 'inline-block' }}
              ...
              <button
                ...
                style={{ padding: '13px 28px', border: '1.5px solid #0d1a0d', color: '#0d1a0d', background: 'transparent', fontSize: '0.85rem', fontWeight: 500, letterSpacing: '0.04em', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', transition: 'border-color .18s,color .18s' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = '#b5f02a'; e.currentTarget.style.color = '#1a5c0a' }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = '#0d1a0d'; e.currentTarget.style.color = '#0d1a0d' }}
              >
        ...
        {/* ══ BOTTOM STRIP ══ */}
        <div className="bottom-strip" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', minHeight: 190 }}>
          <div style={{ background: '#080f08', padding: '36px 48px', display: 'flex', alignItems: 'center', borderTop: '1px solid rgba(181,240,42,0.07)' }}>
            <p style={{ fontSize: '0.78rem', fontWeight: 500, color: 'rgba(244,244,240,0.5)', textTransform: 'uppercase', letterSpacing: '0.1em', lineHeight: 1.9, maxWidth: 240 }}>
          ...
          <div style={{ background: '#f4f4f0', color: '#0d1a0d', padding: '36px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 12 }}>
            <h3 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.5rem', lineHeight: 1.1, letterSpacing: '0.02em', textTransform: 'uppercase' }}>Modern Agriculture Solutions</h3>
            <p style={{ fontSize: '0.82rem', color: '#444', lineHeight: 1.7, maxWidth: 280 }}>We are dedicated to transforming agriculture with modern technology, innovation, and expert guidance for every farmer in Odisha.</p>
          </div>
        </div>

        {/* ══ CIRCULAR GALLERY ══ */}
        <div className="gallery-section" style={{ paddingTop: '80px', paddingBottom: '20px' }}>
          <div className="gallery-header">
            <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.2rem,5vw,4rem)', textTransform: 'uppercase', letterSpacing: '0.02em', color: '#f4f4f0', whiteSpace: 'nowrap' }}>From the Fields</h2>
            <div style={{ flex: 1, height: 1, background: 'rgba(244,244,240,0.08)', alignSelf: 'center' }} />
            <span style={{ fontSize: '0.72rem', color: 'rgba(244,244,240,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Scroll to explore</span>
          </div>
          <CircularGallery items={GALLERY_ITEMS} bend={2} textColor="#b5f02a" borderRadius={0.06} font="bold 26px sans-serif" scrollSpeed={2.5} scrollEase={0.06} />
        </div>
        ...
        <section className="masonry-section" style={{ padding: '96px 48px', background: '#080f08', borderTop: '1px solid rgba(181,240,42,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 20, marginBottom: 56 }}>
            <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.2rem,5vw,4rem)', textTransform: 'uppercase', letterSpacing: '0.02em', whiteSpace: 'nowrap' }}>Why Farmers Trust Us</h2>
          ...
          <div className="masonry-grid">
            {MASONRY.map((item, i) => <MasonryCard key={i} item={item} delay={Math.min(i % 4, 3) * 75} />)}
          </div>
        </section>
        ...
        <section className="footer-cta" style={{ padding: '80px 48px', background: '#b5f02a', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, textAlign: 'center' }}>
          <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.5rem,6vw,5rem)', color: '#0d1a0d', textTransform: 'uppercase', letterSpacing: '0.02em', lineHeight: 1 }}>Ready to Grow Smarter?</h2>
          <p style={{ fontSize: '0.95rem', color: 'rgba(13,26,13,0.7)', maxWidth: 400, lineHeight: 1.78 }}>Join thousands of Odisha farmers already using KrishiMitra to protect their crops and livestock.</p>
          <Link href="/livestock"
      ...
      {videoOpen && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}
        >
          <div onClick={(e) => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 960, borderRadius: 16, overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.6)' }}>
            <video
              src="/tutorial.mp4"
              controls
              autoPlay
            ...
            <button
              onClick={() => setVideoOpen(false)}
              style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(0,0,0,0.65)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', borderRadius: '50%', width: 38, height: 38, fontSize: '1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}
            >
              ✕
            </button>
          </div>
