...
import { useState, useEffect, useRef, useMemo, useId, FC, PointerEvent } from 'react'
...
class App {
  ...
  onCheckDebounce: any; renderer!: Renderer; gl!: GL; camera!: Camera
  scene!: Transform; planeGeometry!: Plane; medias: Media[] = []
  mediasImages: any[] = []; screen!: any; viewport!: any; raf = 0
...
//  CurvedLoop marquee
// ══════════════════════════════════════════════════════
interface CurvedLoopProps { marqueeText?: string; speed?: number; curveAmount?: number; direction?: 'left'|'right'; interactive?: boolean }
const CurvedLoop: FC<CurvedLoopProps> = ({ marqueeText = '', speed = 2, curveAmount = 0, direction = 'left', interactive = true }) => {
  ...
  const uid = useId(); const pathId = `curve-${uid}`
  ...
  const onPointerMove = (e: PointerEvent) => {
    ...
    const cur = parseFloat(textPathRef.current.getAttribute('startOffset') || '0')
    let next = cur + dx
    if (next <= -spacing) next += spacing; if (next > 0) next -= spacing
    textPathRef.current.setAttribute('startOffset', next + 'px'); setOffset(next)
  }
...
const OUTLINE = '1px 1px 0 #000,-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000,0 0 12px rgba(0,0,0,0.95)'
...
export default function HomePage() {
  ...
  const wordAnim: React.CSSProperties = {
    ...
    transition: leaving ? 'transform .42s cubic-bezier(.55,0,.78,0),opacity .35s ease'
      : entered ? 'transform .56s cubic-bezier(.22,1,.36,1),opacity .44s ease' : 'none',
  }
  const langAnim: React.CSSProperties = {
    ...wordAnim,
    transition: leaving ? 'transform .42s .04s cubic-bezier(.55,0,.78,0),opacity .35s ease'
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 900px) {
          ...
          .marquee-strip   { height: 60px; padding-top: 6px; }
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", background: '#0d1a0d', color: '#f4f4f0', minHeight: '100vh', overflowX: 'hidden' }}>
        ...
        <div className="marquee-strip">
          <CurvedLoop
            ...
            speed={1.4} curveAmount={0} direction="left" interactive={true}
          />
        </div>
        ...
        <section className="hero-section hero-bg" style={{ position: 'relative', minHeight: 'calc(100vh - 72px)', display: 'grid', gridTemplateColumns: '1fr 1fr', padding: '0 48px', overflow: 'hidden' }}>
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 0 80px', gap: 28 }}>
            <div className="heading-row" style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
              <h1 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(4rem,10vw,9.5rem)', lineHeight: 1, letterSpacing: '0.01em', color: '#f4f4f0', textTransform: 'uppercase', whiteSpace: 'nowrap', textShadow: OUTLINE }}>
                KRISHI
              </h1>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
                <div style={{ overflow: 'hidden', height: 'clamp(3.8rem,9.8vw,9rem)' }}>
                  <span key={`w-${index}`} style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(3.8rem,9.8vw,9rem)', lineHeight: 1, color: '#b5f02a', textTransform: 'uppercase', letterSpacing: '0.01em', textShadow: OUTLINE, ...wordAnim }}>
                    {WORDS[index].word}
                  </span>
                </div>
            ...
            <p style={{ fontSize: '0.95rem', color: 'rgba(244,244,240,0.88)', maxWidth: 360, lineHeight: 1.78, fontWeight: 300, textShadow: '0 1px 8px rgba(0,0,0,0.8)' }}>
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              ...
              <Link href="/crop"
                style={{ padding: '13px 28px', border: '1.5px solid rgba(244,244,240,0.5)', color: '#f4f4f0', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 500, letterSpacing: '0.04em', display: 'flex', alignItems: 'center', gap: 8, transition: 'border-color .18s,color .18s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = '#b5f02a'; e.currentTarget.style.color = '#b5f02a' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(244,244,240,0.5)'; e.currentTarget.style.color = '#f4f4f0' }}>
          ...
          <div className="right-col" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-end', padding: '60px 0 80px' }}>
            <div style={{ position: 'relative', width: 300, height: 400, marginRight: 40 }}>
              <div style={{ position: 'absolute', top: -22, left: -22, width: '100%', height: '100%', background: '#b5f02a', zIndex: 0 }} />
              <img src="/farmer.jpg" alt="Farmer" style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            </div>
            <div style={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.5rem', color: '#f4f4f0', letterSpacing: '0.02em', textShadow: '0 1px 8px rgba(0,0,0,0.9)' }}>10,000+</span>
                <span style={{ fontSize: '0.65rem', fontWeight: 500, color: 'rgba(244,244,240,0.8)', textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: 'right', lineHeight: 1.4 }}>Farmers<br />Benefited</span>
              </div>
              <div className="stat-vert" style={{ background: 'rgba(13,26,13,0.9)', border: '1px solid rgba(181,240,42,0.3)', padding: '12px 8px', fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#b5f02a' }}>
        ...
        {/* ══ CIRCULAR GALLERY ══ */}
        {/* paddingTop: 80px = more space above header | paddingBottom: 20px = tighter at bottom */}
        <div className="gallery-section" style={{ paddingTop: '80px', paddingBottom: '20px' }}>
          <div className="gallery-header">
            <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.2rem,5vw,4rem)', textTransform: 'uppercase', letterSpacing: '0.02em', color: '#f4f4f0', whiteSpace: 'nowrap' }}>
              From the Fields
            </h2>
        ...
        <section className="masonry-section" style={{ padding: '96px 48px', background: '#080f08', borderTop: '1px solid rgba(181,240,42,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 20, marginBottom: 56 }}>
            <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.2rem,5vw,4rem)', textTransform: 'uppercase', letterSpacing: '0.02em', whiteSpace: 'nowrap' }}>
